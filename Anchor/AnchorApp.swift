...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let anchorModel: AnchorModel = .shared
    ...
    func updateStatusItemDisplay() {
        let timerString = self.anchorModel.timerDisplayString
        self.menuBarLabel?.rootView = MenuBarLabel(task: self.anchorModel.currentTask, timerString: timerString)
        self.statusItem.length = self.labelWidth(for: self.anchorModel.currentTask, timerString: timerString)
    }

    // MARK: - Observation of AnchorModel

    func startObservingModel() {
        withObservationTracking {
            _ = self.anchorModel.currentTask
            _ = self.anchorModel.timeRemaining
        } onChange: {
            Task { @MainActor in
    ...
    private func setupStatusItem() {
        ...
        let label = NSHostingView(rootView: MenuBarLabel(task: anchorModel.currentTask))
...
extension AppDelegate: NSMenuDelegate {
    func menuNeedsUpdate(_ menu: NSMenu) {
        ...
        let setTaskTitle = self.anchorModel.currentTask.isEmpty ? "Set Task…" : "Change Task…"
        ...
        if !self.anchorModel.currentTask.isEmpty {
        ...
        if !self.anchorModel.currentTask.isEmpty || self.anchorModel.isTimerRunning {
